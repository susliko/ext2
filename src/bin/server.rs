...
fn handle_client(fs: &mut Fs, stream: &mut TcpStream) -> std::io::Result<()> {
  ...
  writer
    .write(b"Welcome to a modest ext2-like file system!. Type `help` to list its capabilities.\n")?;
...
fn bind_and_handle(port: usize) -> std::io::Result<()> {
...
fn main() {
  bind_and_handle(4242).map_or_else(
    |e| println!("Error while handling socket: {:?}", e),
    |_| ()
  )
}
